...
- **Connection** — The linkage between a User and an external third-party app for data and actions (e.g. Google Calendar, Slack). Distinct from Account: deleting a Connection never affects sign-in, and vice versa. Each Connection holds its own scopes and tokens. Surfaced in the UI as "Apps" (route: `/apps`). See `docs/adr/0001-separate-account-from-connection.md`.
